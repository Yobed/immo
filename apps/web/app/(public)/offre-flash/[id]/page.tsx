...
export default async function OffreFlashDetailPage({ params }: PageProps) {
  ...
  // Admin : contacts DIRECTS — propriétaire/annonce (telephone_bien) ET source
  // qui a posté l'annonce (telephone_expediteur). Colonnes normalement masquées
  // pour le public : on ne les fetch QUE pour un admin.
  let flashBienPhone: string | null = null
  let flashBienName: string | null = null
  let flashSourcePhone: string | null = null
  let flashSourceName: string | null = null
  if (isAdminUser) {
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const { data: contactRow } = await (locaux as any)
      ...
      .single()
    if (contactRow) {
      flashBienPhone = contactRow.telephone_bien || null
      flashBienName = contactRow.publie_par || null
      flashSourcePhone = contactRow.telephone_expediteur || null
      flashSourceName = contactRow.expediteur || null
    }
  }
  // Ligne de contact réutilisable (nom + numéro + WhatsApp/Appeler)
  const contactLine = (label: string, name: string | null, phone: string) => (
    <div>
      <p className="text-[10px] font-bold uppercase tracking-widest text-red-700/70">{label}</p>
      {name && <p className="text-sm font-bold text-[var(--text)]">{name}</p>}
      ...
      <div className="flex gap-2 mt-1.5">
        ...
        <a
          href={`tel:${phone}`}
          className="flex-1 inline-flex items-center justify-center gap-1.5 py-2 rounded-lg bg-slate-900 text-white text-xs font-bold hover:bg-slate-800 transition-colors"
        >
  ...
  const flashAdminContact = isAdminUser ? (
    <div className="mb-3 p-3.5 rounded-xl bg-red-500/5 border border-red-500/25 space-y-3">
      <p className="text-[10px] font-bold uppercase tracking-widest text-red-700 inline-flex items-center gap-1">
        <ShieldCheck className="w-3 h-3" /> Contacts · admin
      </p>
      {flashBienPhone && contactLine('Propriétaire / annonce', flashBienName, flashBienPhone)}
      {flashSourcePhone && flashSourcePhone !== flashBienPhone && contactLine('Posté par (source)', flashSourceName, flashSourcePhone)}
      {!flashBienPhone && !flashSourcePhone && (
        <p className="text-xs text-[var(--text-muted)]">Aucun numéro dans l’annonce scrapée.</p>
      )}
    </div>
  ) : null
