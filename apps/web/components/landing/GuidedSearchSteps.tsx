      ...
      <div className="max-w-[1200px] mx-auto px-4 md:px-6">
        ...
        <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6 mb-8 md:mb-10">
          ...
          <button
            ...
            className="inline-flex items-center gap-2 self-start md:self-auto min-h-[48px] px-5 py-3 rounded-full bg-[var(--accent-luxury)] text-[var(--on-accent)] font-bold text-sm hover:opacity-90 active:scale-95 transition-all shadow-md"
          >
        ...
        <ol className="grid grid-cols-1 md:grid-cols-3 gap-3 md:gap-0 md:bg-[var(--surface-card)] md:rounded-3xl md:border md:border-[var(--border)] md:overflow-hidden">
          {steps.map((s, i) => (
            <motion.li
              ...
              className={`relative p-5 md:p-6 bg-[var(--surface-card)] border border-[var(--border)] rounded-2xl md:rounded-none md:border-0 ${
                i > 0 ? 'md:border-l md:border-[var(--border)]' : ''
              }`}
            >
              {/* Number badge */}
              <div className="flex items-center gap-3 mb-3">
                <span className="inline-flex items-center justify-center w-9 h-9 rounded-full bg-[var(--accent-luxury-muted)] text-[var(--accent-luxury)] font-display font-bold text-base">
                  {s.n}
                </span>
        ...
        {/* Reassurance line — next step explicite */}
        <p className="mt-6 text-center text-xs text-[var(--text-muted)]">
          <span className="inline-flex items-center gap-1.5">
            <Heart className="w-3 h-3 text-[var(--accent-luxury)]" />
