...
const BADGE_CLASSES: Record<MediaType, string> = {
  photo:   BADGE_BASE,
  video:   BADGE_BASE,
  vue_360: 'bg-[var(--accent-luxury-muted)] text-[var(--accent-luxury)] border border-accent-luxury/20',
  plan:    BADGE_BASE,
}
            ...
            return (
              <button
                ...
                className={cn(
                  ...
                  activeFilter === filter
                    ? 'bg-[var(--accent-luxury)] text-[var(--text)] border-[var(--accent-luxury)] scale-105 shadow-[0_0_20px_rgba(212,175,55,0.3)]'
                    : 'bg-black/80 text-white border-white/20 hover:border-accent-luxury/50 hover:bg-black'
                )}
              >
      ...
      <div className={cn("relative group", isHero && "h-full")}>
                ...
                <div className={cn("relative", isHero && "h-full")}>
                  ...
                  {!isHero && (
                    <div className="absolute top-4 right-4 pointer-events-none">
                      <span className="inline-flex items-center px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest bg-background/90 text-white border border-white/20 backdrop-blur-xl shadow-md">
                  ...
                  {media.titre && !isHero && (
                    <div className="absolute bottom-4 left-4 right-4 pointer-events-none">
                      <span className="inline-flex items-center px-4 py-2 rounded-xl text-[10px] font-medium tracking-wide bg-background/90 text-white border border-white/10 backdrop-blur-xl shadow-md">
                        {media.titre}
                      </span>
                    </div>
                  )}
        ...
        {filtered.length > 1 && (
          <>
            <button
              onClick={scrollPrev}
              className="absolute left-6 top-1/2 -translate-y-1/2 w-14 h-14 rounded-full bg-background/40 hover:bg-background/80 backdrop-blur-xl border border-white/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500 hover:scale-110 shadow-md z-40 pointer-events-auto"
            ...
            <button
              onClick={scrollNext}
              className="absolute right-6 top-1/2 -translate-y-1/2 w-14 h-14 rounded-full bg-background/40 hover:bg-background/80 backdrop-blur-xl border border-white/20 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-500 hover:scale-110 shadow-md z-40 pointer-events-auto"
              aria-label="Suivant"
            >
