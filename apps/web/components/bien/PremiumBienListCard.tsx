...
interface PremiumBienListCardProps {
  id: string
  titre: string
  commune: string
...
export function PremiumBienListCard({
  ...
  return (
    ...
    >
      ...
      >
        <Link 
          href={`/biens/${id}`} 
          className="flex flex-[1_1_auto] w-full h-full no-underline"
          onClick={(e) => {
            if (onSelect) {
              e.preventDefault();
            }
        ...
        <div className={`relative flex flex-col sm:flex-row w-full h-full bg-[var(--surface-card)] rounded-[1.5rem] overflow-hidden border transition-all duration-700 hover:shadow-[0_32px_64px_-16px_rgba(0,0,0,0.12)] hover:-translate-y-2 group-active:scale-[0.98] ${isSelected ? 'border-[var(--accent-luxury)] ring-1 ring-[var(--accent-luxury)]' : 'border-[var(--border)]'}`}>
          ...
          <div className="flex-1 flex flex-col p-6 lg:p-8 bg-inherit">
