...
export function Breadcrumb({ items, className = '', variant = 'dark' }: Props) {
  const textBase = variant === 'dark' ? 'text-slate-500' : 'text-white/50'
  const textActive = variant === 'dark' ? 'text-slate-900' : 'text-white'
  const textHover = variant === 'dark' ? 'hover:text-slate-700' : 'hover:text-white/80'
  const sepColor = variant === 'dark' ? 'text-slate-300' : 'text-white/20'
